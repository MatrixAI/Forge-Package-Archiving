...
-- Make the server with stdLogger, port 8888
main = serverWith defaultConfig { srvLog = stdLogger, srvPort = 8888 }
    --Anonymous function for the handler to which is used to process requests
     $ \_ url request ->
  --Get the request method out of the request and case on it:
  case rqMethod request of
    --if the request method is GET
    GET ->
      --parse the extension from the url_path
      do let ext = takeExtension (url_path url)
         --Attempt to read in a page located at the url path
